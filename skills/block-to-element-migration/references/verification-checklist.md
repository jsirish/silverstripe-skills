...
- [ ] **Strip dynamic noise** — server-rendered timestamps, CSRF tokens, asset hashes, dev-navigator block (`<div Live>...</div>`). The `example-custom-data-migration` skill has a `strip_dynamic_bits.sh` helper.
